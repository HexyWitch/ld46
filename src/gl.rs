...
impl Context {
    ...
    pub fn create_shader(&mut self, shader_type: ShaderType, src: &str) -> Result<Shader, GLError> {
        unsafe {
            let shader_id = self
                .context
    ...
    pub fn create_program(&mut self, desc: &ProgramDescriptor) -> Result<Program, GLError> {
        unsafe {
    ...
    pub fn create_vertex_buffer(&mut self) -> Result<VertexBuffer, GLError> {
        unsafe {
...
impl VertexBuffer {
    pub fn write<V: AsBytes>(&mut self, vertices: &[V]) {
...
impl Texture {
    pub fn write(&mut self, x: u32, y: u32, width: u32, height: u32, data: &[u8]) {
        unsafe {
            self.context
...
impl Program {
        ...
        unsafe {
            self.set_uniforms[index].1 = match value {
                Uniform::Texture(texture) => {
                    Some(SetUniformValue::Texture(texture.texture_id.clone()))
                }
    ...
    pub fn render_vertices(&self, vertex_buffer: &VertexBuffer) -> Result<(), GLError> {
        unsafe {
            self.context
